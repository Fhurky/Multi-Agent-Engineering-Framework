...
| F-058-03 | Caller-computable manifest/snapshot/integration sets narrowed the claimed universe, and a fabricated nonexistent production decision admitted. | `admission.ts`: `artifactAuthentic`, complete `enumerateAdmissionUniverse` comparison, accepted-human resolution, and `resolveHumanDecision`; `gate-admissibility.ts`: authenticated decision facts consumed only after universe checks | `task-059-remediation.test.ts`: narrowed gate universe and unresolvable human decision cases | Narrowed/resealed relations returned `SourceRecordInvalid`; the unresolvable decision returned one unclassifiable human exception and could not authorize coupling. |
...
- `ReleaseAuthorityPort` is a separately injected, read-only resolver. It authenticates
  ...
  publication execution identities. It exposes no mutation operation.
