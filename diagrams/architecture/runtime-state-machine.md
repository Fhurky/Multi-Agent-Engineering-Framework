...
Source diagrams for the run and task state machines. Produced under TASK-002 and amended under TASK-016, TASK-024, TASK-028, and TASK-036.
...
Amended by TASK-024 for the terminal-task evidence exceptions, by TASK-028 for the blocked-drain attach path, recovery of unverified process closures, result-effect ordering, and the canonical `ReconciliationInput`, and by TASK-036 for decision-block sequential legality and the exact recovery-completion event.
...
Recovery constructs exactly one canonical `ReconciliationInput` per task and emits exactly one decision block whose fixed event sequence is already present above. The first event is legal from restored state and later events are applied sequentially inside the same block; blocks are independent and never interleaved. ADR-0030 supersedes ADR-0013's former input-count wording; the type and its sole cardinality statement are normative there. Result adoption is legal only when the matching attempt already has `WorkerResultRecorded`, exactly one `EffectIntentRecorded{isTaskResultEffect:true}`, and `EffectCommitted`; recovery then appends only `WorkerSucceeded`.
...
The `retry_timeout` row intentionally contains two events addressed to the same task. It is one decision block: `TaskTimedOut` first enters `awaiting_retry`, then `RetryScheduled` is legal in that resulting state. Recovery ends with the exact `RunRecoveryCompletedEvent`, carrying all five canonical evidence arrays, as the final event of the batch.
