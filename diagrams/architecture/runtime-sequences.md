...
Source diagrams for the eight sequences that carry the runtime's guarantees. Produced under TASK-002 and amended under TASK-016, TASK-024, and TASK-028.
...
sequenceDiagram
  ...
  Sup->>WS: completeFinalize(plan, publication, publicationReceipt)
...
sequenceDiagram
  ...
  Rec->>Rec: Phase 4 — construct one canonical ReconciliationInput<br/>per task, then classify it once<br/>(workspace findings inform construction, not a second pass)
...
sequenceDiagram
  ...
  Inbox-->>Collector: appended or deduplicated entry, highWaterMark
  Collector-->>Sup: PreDispatchCollectResult{highWaterMark}
  ...
  Sup->>Store: append ONE batch: [...effects,<br/>IngressRangeConsumed{rows from delivery},<br/>WorkerSucceeded]
  Note over Store: ledger rows and cursor move together.<br/>If this batch does not commit, the same immutable<br/>delivery range is read again after recovery
...
Seven properties are visible here:
