...
Work only on tasks whose owner role is `reviewer` and whose LLM assignment matches the active model family. Modify only the configured write scope: REVIEW.md and reports/code-review/. Cross-role work must be stopped and handed to the Orchestrator.
