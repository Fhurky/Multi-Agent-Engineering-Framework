...
**Revision 16**, produced by TASK-013 activation `ACT-015`. **No verdict drove it and no gate is closed by it.** It consumed one `artifact_published` fact:
...
| TASK-018 | Runtime toolchain bootstrap | devops | claude | `LIN-ARCH-REVIEW` `gate_passed(review, 8)` | blocked |
...
Wave 1   TASK-039                            gate the seventh amendment before it merges, ready now
         TASK-038                            seventh architecture amendment, published local-only at 8ea5c32, review, unjudged, not integrable
...
**No approved architecture source exists.** Every round of this lineage has returned `changes-required`: rounds 1 … 7 rejected `9576fc9`, `8d0c570`, `c2ee3eb`, `fe0374c`, `468b37b`, `6d145eb`, and `970b081`. **The architecture this graph will build on is `9576fc9` as amended by `8d0c570`, by `c2ee3eb`, by `fe0374c`, by `468b37b`, by `6d145eb`, by `970b081`, and further amended by the TASK-038 commit that TASK-039 approves** — and it comes into being only when `LIN-ARCH-REVIEW` records a passing or formally accepted authoritative verdict at `lineage_round` 8 or later. Until then the graph names a *target*, not an approved source. Every architecture dependency edge names the lineage `LIN-ARCH-REVIEW` at `lineage_round: 7` rather than any single task, so no further retarget is required if an eighth round becomes necessary — only the floor moves, and under F-601 it never moves without the source clause moving in the same commit.

**Normative source rule.** F-202 recorded that TASK-003 still pinned the rejected baseline alone; **F-601 then recorded that the nine consumers named the round-3 amendment as one TASK-025 approves, when TASK-025 had rejected it.** Every implementation record now states its normative source in the same corrected form: *the named documents at `9576fc9` **as amended by `8d0c570`, by `c2ee3eb`, by `fe0374c`, by `468b37b`, by `6d145eb`, by `970b081`, and by the TASK-038 commit that TASK-039 approves***, together with the explicit statement that none of the seven named baselines is approved. A record that cites a baseline without the amendment clause is a finding; a record that names `8d0c570`, `c2ee3eb`, `fe0374c`, `468b37b`, `6d145eb`, or `970b081` as approved is a finding; and a record that attributes an approval to a round that recorded `changes-required` is a finding. No superseded baseline and no ungated amendment is ever the normative source on its own. **A raised lineage floor is not by itself sufficient** — F-601 recorded that the floor was mechanically correct at round 4 while the source clause still pointed at the rejected round-3 artifact, so the floor and the source clause must be moved together.
