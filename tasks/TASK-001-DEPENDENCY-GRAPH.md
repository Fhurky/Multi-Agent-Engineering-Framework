...
**Revision 19**, produced by TASK-013 activation `ACT-018`. **No verdict drove it and no gate was closed by it.** It consumed one `artifact_published` fact and routed one new user requirement:
...
| **TASK-040** | Architecture for autonomous post-gate integration authority | architect | gpt | **`HUMAN-004` `human_decision`** | **blocked** |
| **TASK-041** | Independent review of the integration-authority amendment | reviewer | gpt | TASK-040 `review_ready` | **blocked** |

### `HUMAN-004` — autonomous post-gate integration authority, OPEN

**Registered at revision 19 by activation `ACT-018`. It is a question, not a decision, and nothing in this graph presupposes an answer.**

**The requirement.** The user stated in the control session that routine human GitHub merging must become **zero**: after the required independent gates pass, the system should automatically squash-merge the approved task pull request into `integration/autonomous-runtime`, record `branch_integrated`, advance lifecycle state, and dispatch the work that becomes ready. Humans would remain only for explicit policy exceptions — the named examples are formal acceptance of a blocking High or Critical security risk, and authorization of an irreversible production action.

**Why it is a governance decision rather than a task the Orchestrator can route straight to an implementer.** The Orchestrator checked the approved architecture at `8ea5c32`, as integrated at `de3a8d6`, and it grants no such authority:
...
**What the decision must settle.** Whether a non-human component may perform the merge at all; which component holds the authority and under what module ownership; the exact residual human exception set and how a component detects it is inside it; and which governance artefacts change to permit it — at minimum `AGENTS.md`'s mandatory protocol, and possibly `config/agents/settings.yaml`, both of which are human-controlled and unmodifiable from an `agent/*` branch.

**What was routed, and what deliberately was not.** The architecture half is **TASK-040**, `blocked` on `human_decision(HUMAN-004)` — the second `human_decision` edge this graph has carried, after TASK-018's on `HUMAN-001` — gated by **TASK-041** under the new lineage `LIN-INTEGRATION-AUTHORITY-REVIEW`. **No implementation task exists and none may be created** until TASK-041 records a passing verdict on an amendment authored under a recorded decision. **No safeguard was weakened to accommodate the requirement**: no structural prohibition, no gate, no verdict, no write scope, and no publication rule changed at revision 19.

**It is not an ingress fact and raises no `ingress_seq`.** Like `HUMAN-002`, it has no durable governance commit; unlike `HUMAN-001` at `fb9f45c` and `HUMAN-003` at `0b413b7`, there is nothing to hash. Recording the decision in a tracked commit on a non-agent branch is what would make it one, and that commit is the ingress fact that unblocks TASK-040. **A decision that refuses the change closes TASK-040 rather than blocking it**, and that outcome is as legitimate as approval.

**TASK-018 and TASK-019 depend on none of this.** Neither is delayed, accelerated, or re-gated by it, and **TASK-018 must never be merged before TASK-019 records a passing verdict** whatever `HUMAN-004` decides.
...
**Seventy-four** `gate_for` / `gate_tasks` pairs are declared at revision 19 — **73 before this revision, plus the one `LIN-INTEGRATION-AUTHORITY-REVIEW` round-1 relation TASK-041 carries.** The total is stated by enumeration over the committed records on both sides independently so that it can be recomputed rather than trusted, and **no existing pair's verdict changed at revision 19**: TASK-018's `review` and `security` relations both stay `pending`, and all eight `LIN-ARCH-REVIEW` round-8 relations stay closed at `approved`.
...
Wave 2a  TASK-040                            integration-authority architecture, BLOCKED on the open decision HUMAN-004
...
**Ownership gaps.** All are routed and all appear in the graph. **Revision 19 adds one**, and it is the first gap opened by a new user requirement rather than by a review finding:

| Gap | Routed to | Status |
|---|---|---|
| **No component may merge an approved task branch into `integration/autonomous-runtime`** | **`HUMAN-004`** for the authority; **TASK-040** for the architecture, gated by **TASK-041**; no implementation owner yet | **Open, and deliberately not filled.** The eight-module map assigns the integration merge to no module; `INTEGRATION-STRATEGY.md` prescribes the method and order with the operator as executor; the workspace module's single push function is structurally pinned to its own task branch and takes no ref parameter; and the `branch_integrated` ingress class names a "runtime integration step" producer that has never existed. `AGENTS.md` additionally requires each agent to use a pull request and reserves privileged, external, and release-changing actions for human approval. So the gap has **two** prerequisites, a governance decision and an architecture amendment, and the Orchestrator routed both rather than closing either. **No implementation task exists and none may be created** until TASK-041 records a passing verdict on an amendment authored under a recorded `HUMAN-004`. Nothing about TASK-018 or TASK-019 depends on this |
...
7. **No owner merges anything.** Opening or updating a pull request is where an owner's authority ends. The merge into `integration/autonomous-runtime` is a separate externally visible operation performed by the operator, only after every gate in the target's `pre_merge_gates` is closed, and it produces its own `branch_integrated` ingress fact. Whether any non-human component may ever perform it is the open decision **`HUMAN-004`**; until that is recorded and TASK-041 passes, this step is unchanged and no task may act as though it were.

Steps 1 through 5 are what TASK-017 automates for runtime-dispatched work, including step 4's publication and pull-request outcome. They remain the manual procedure for the human-launched CLI sessions that build the runtime itself. **Step 7 is automated by nothing today.**
