...
**Revision 21**, produced by TASK-013 activation `ACT-020`. **No verdict drove it and no gate was closed by it.** It consumed one `artifact_published` fact:
...
| **`LIN-INTEGRATION-AUTHORITY-REVIEW`** | review | TASK-040 | 1 → TASK-041 (TASK-040 r1) |
...
- Invariant 3: checked pairwise across all **74** pairs in the gate assignment table and both registers, by enumeration over the committed records rather than by inheriting a total.
- Invariant 4: the only non-empty `pre_merge_gates` are `review` on TASK-002, TASK-016, TASK-024, TASK-028, TASK-032, TASK-034, TASK-036, and TASK-038 — the eight `LIN-ARCH-REVIEW` cohort members, all closed at round 8 — plus `review` on TASK-018 (TASK-019 r1) and `review` on **TASK-040** (TASK-041 r1). None of TASK-015, TASK-019, TASK-020, TASK-025, TASK-029, TASK-033, TASK-035, TASK-037, TASK-039, or TASK-041 holds an `integrated` edge to any of its targets.
...
- Invariant 7: all **74** `gate_for` / `gate_tasks` pairs carry both properties and agree on both across the pair, checked by enumerating the committed records on each side independently rather than by reading one side and trusting the other. Each declared `retrospective` was recomputed as `gate ∉ target.pre_merge_gates`: **`false` for 38** — the 36 `LIN-ARCH-REVIEW` review pairs, whose gate is a pre-merge gate on every cohort member; TASK-019 r1 on TASK-018; and TASK-041 r1 on TASK-040 — and **`true` for 36**, which are the 28 runtime and toolchain assembly gates and the eight rounds of TASK-001's own review gate, since TASK-001 declares `pre_merge_gates: []`. Each declared `gate_class` was recomputed against the publication of the artifact that round reviews: **`point` for 46** — 36 architecture, 8 decomposition, 1 toolchain review, and 1 integration-authority review — and **`aggregate` for 28**, which are the 8 `LIN-RUNTIME-REVIEW`, 8 `LIN-RUNTIME-SECURITY`, 8 `LIN-RUNTIME-QA`, and 3 `LIN-RUNTIME-PERFORMANCE` assembly pairs plus the 1 `LIN-TOOLCHAIN-SECURITY` pair. 38 + 36 = 74 and 46 + 28 = 74, so both partitions account for every pair. **Revision 19 corrects this passage**, which revision 13 had brought to 52 and which revisions 15 and 17 then left behind as the total grew to 73; invariant 3 in the same list already said 73, so the document contradicted itself for two revisions. That is the A-506 defect class recurring, and it is recorded rather than quietly repaired — see the `ACT-018` section of `tasks/TASK-013-ACTIVATION-LOG.md`.
- Invariant 8: **nine** lineages are registered. `LIN-DECOMP-REVIEW` declares rounds 1 … 8 with no gap, one task per round except the historical TASK-014 pair at rounds 1 and 2, which is recorded as the single pre-rule exception. `LIN-ARCH-REVIEW` declares rounds 1 … 8 with no gap, one task each, and its cohort reached eight members at round 8 with none ever removed. The remaining seven — including the new `LIN-INTEGRATION-AUTHORITY-REVIEW` — declare round 1 only. Every target named by a pair is a member of its lineage's registered cohort. Every lineage round greater than 1 follows a recorded verdict: `LIN-DECOMP-REVIEW` rounds 2 … 8 follow `changes-required` at `8ac0dbd`, `abb85d9`, `adfb982`, `e8eb23d`, `667d3b8`, `710351fd`, and `f36e6c06`; `LIN-ARCH-REVIEW` rounds 2 … 8 follow `changes-required` at `8632469`, `4874a9d`, `aa38c7d2`, `3df261fa`, `3660cc2`, `afed101`, and `9bb75d9`, and round 8 itself recorded **`approved`** at `734bdbc`. **`LIN-ARCH-REVIEW` round 9 is deliberately not declared**: revision 19's new architecture work is a new artifact rather than a remediation of a failing round, so it opens a new lineage instead — see the register above. **`LIN-DECOMP-REVIEW` round 9 is still not declared**: round 8 has recorded no verdict, and this invariant forbids declaring a round before the previous one records one. That is why the A-506 correction's own review, and revision 19's corrections, are a recorded obligation rather than an existing task — see the reconciliation section.
...
| **TASK-040** | Architecture for autonomous post-gate integration authority, **both executors** | architect | gpt | **`HUMAN-004` `human_decision` — satisfied** at `7dc0748` | **review**, published at `5e5fc8f`, pull request 22 open and **unjudged** |
| **TASK-041** | Independent review of the integration-authority amendment | reviewer | gpt | **TASK-040 `review_ready` — satisfied** at `5e5fc8f` | **ready** |
...
**What is routed, and what deliberately is not.** The architecture half is **TASK-040**, which **published at `5e5fc8f` and is now `review`**, covering **both** executors, gated by **TASK-041** under `LIN-INTEGRATION-AUTHORITY-REVIEW` round 1 — **`ready` and dispatchable at revision 21**, with its review scope matching. **No implementation task exists and none may be created** until TASK-041 records a passing verdict — which is both this graph's gate rule and the decision's own fourth sequencing step. When they are created there are **two**, owned separately by `runtime` and `devops`, plus the reviewer, security, QA, and failure-injection validations the decision names. **Publication is not approval**: the amendment exists, and nothing has judged whether it stays inside the boundaries `HUMAN-004` sets. **No safeguard was weakened to accommodate the decision**: no structural prohibition, no gate, no verdict, no write scope, and no publication rule changed at revision 20 or revision 21, and no governance or enforcement file was edited by any agent.

**The `AGENTS.md` amendment is authorized and is nobody's to author here.** `AGENTS.md` is a human-controlled enforcement path and cannot be changed from an `agent/*` branch; it is outside every agent role's write scope, including the architect's. TASK-040 states what the amendment must say and returns it, and the user performs it. The same holds for branch protection, required checks, merge permissions, and the least-privilege credential.
...
| **TASK-040** | review | TASK-041 r1 | pending — **dispatchable now**; blocks the merge of pull request 22 | `LIN-INTEGRATION-AUTHORITY-REVIEW` | 1 |

**Seventy-four** `gate_for` / `gate_tasks` pairs are declared at revision 21, **unchanged from revisions 20 and 19**, the last of which had reached 74 from 73 by adding the one `LIN-INTEGRATION-AUTHORITY-REVIEW` round-1 relation TASK-041 carries. Revision 21 creates no gate task and no gate relation. The total is stated by enumeration over the committed records on both sides independently — 74 `gate_for` entries and 74 `gate_tasks` entries at `ACT-020` — so that it can be recomputed rather than trusted, and **no existing pair's verdict changed at revision 21**: TASK-040's `review` relation with TASK-041 stays `pending` and becomes **dispatchable** without becoming judged, TASK-018's `review` and `security` relations both stay `pending`, and all eight `LIN-ARCH-REVIEW` round-8 relations stay closed at `approved`.
...
1. **A pre-merge gate owner is always `point`, and no pre-merge gate is `aggregate`.** TASK-015, TASK-019, TASK-020, TASK-025, TASK-029, TASK-033, TASK-035, TASK-037, TASK-039, and TASK-041 are each dispatchable when the artifact they review publishes, so no gate that blocks integration ever waits several waves. **TASK-019 is the first time this property has been exercised on an implementation artifact**: TASK-018 published at `296b14f` without merging, and its gate became dispatchable immediately. That is finding F-101's correction, working.
2. **A superseding round is a new task, not a re-entrant one, and the relation that survives supersession is the lineage.** TASK-014, TASK-015, TASK-020, TASK-021, TASK-022, TASK-023, TASK-025, TASK-027, TASK-029, TASK-030, TASK-033, TASK-035, TASK-037, and TASK-039 recorded durable verdicts and are `done`. TASK-031 performs decomposition round 8, TASK-019 performs toolchain review round 1, and TASK-041 will perform integration-authority round 1.
...
Wave 2a  TASK-040                            integration-authority architecture, REVIEW - published at 5e5fc8f, PR 22 open and unjudged
         TASK-041                            READY - gates that amendment; PR 22 cannot merge before its verdict; off the runtime critical path
...
**Ownership gaps.** All are routed and all appear in the graph. Revision 19 added one — the first gap opened by a new user requirement rather than by a review finding — revision 20 advanced it by exactly one prerequisite and widened it by one merge, and **revision 21 advances it by one more: the architecture prerequisite is now authored and published, and it is unjudged**:

| Gap | Routed to | Status |
|---|---|---|
| **No component may merge an approved task branch into `integration/autonomous-runtime`, and none may merge that branch into `main`** | **`HUMAN-004`** for the authority — **decided**; **TASK-040** for the architecture of **both** executors, gated by **TASK-041**; no implementation owner yet | **Governance prerequisite discharged, architecture prerequisite open, gap not filled.** The eight-module map assigns the integration merge to no module; `INTEGRATION-STRATEGY.md` prescribes the method and order with the operator as executor; the workspace module's single push function is structurally pinned to its own task branch and takes no ref parameter; the `branch_integrated` ingress class names a "runtime integration step" producer that has never existed; and the `main` merge is additionally what the tracked pre-push hook exists to constrain. `HUMAN-004` at `7dc0748` settled who may hold the authority — a `runtime`-owned integration executor and a `devops`-owned release executor, separately owned, conditionally authorized — and **settled nothing else**. **TASK-040 has now published the amendment for both executors at `5e5fc8f` as pull request 22, and it is `review` and unjudged**; TASK-041 is `ready` and dispatchable, and **no implementation task exists and none may be created** until it records a passing verdict. **The published amendment does not narrow the gap by itself** — an unjudged amendment is a proposal, and the module map, `INTEGRATION-STRATEGY.md`, the pinned push function, and the pre-push hook all still stand exactly as described above until a passing verdict permits them to be changed by an implementation task. The gap itself closes only when both executors are built and independently validated, which is four conditions away by the decision's own count. Nothing about TASK-018 or TASK-019 depends on this |
...
The reviewer-owned report files — TASK-009's two paths, TASK-014, TASK-015, TASK-019, TASK-020, TASK-021, TASK-022, TASK-023, TASK-025, TASK-027, TASK-029, TASK-030, TASK-031, TASK-033, TASK-035, TASK-037, TASK-039, and **TASK-041** — are mutually path-disjoint by construction, so any of them may run concurrently. **TASK-019 and TASK-031 are both `ready` at revision 19 with disjoint report paths and no shared lock**, so they are genuinely parallel.
...
| Validating owner (TASK-009 … TASK-012, TASK-014, TASK-015, TASK-019 … TASK-023, TASK-025, TASK-027, TASK-029, TASK-030, TASK-031, TASK-033, TASK-035, TASK-037, TASK-039, TASK-041) | Its own report scope; the pull request description | Any file under `tasks/` |
| **A human acting on their own authority** | Anything, including files under `tasks/` and `config/` | — |
| Architect (TASK-002, TASK-016, TASK-024, TASK-028, TASK-032, TASK-034, TASK-036, TASK-038, TASK-040) | Its architecture scope; the commit message; the pull request description | Any file under `tasks/` |
