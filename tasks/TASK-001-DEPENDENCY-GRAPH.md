...
**Revision 12**, produced by TASK-013 activation `ACT-011`. **No verdict drove it and no gate is closed by it.** It consumed one `artifact_published` fact:
...
| TASK-034 | Fifth architecture amendment: the round-5 blocking findings | architect | gpt | TASK-033 `gate_recorded` | review, published `local-only` at `6d145eb`, unjudged |
| TASK-035 | Independent review of the fifth architecture amendment | reviewer | gpt | TASK-034 `review_ready` | ready |
...
Wave 1   TASK-035                            gate the fifth amendment before it merges, ready now
         TASK-034                            fifth architecture amendment, published local-only at 6d145eb, review, unjudged, not integrable
...
At revision 12, `architecture-docs` is **free**: the TASK-034 execution's lock was released after its commit, confirmed by a direct read of the shared lock directory at `ACT-011`, which holds exactly one entry, `task-013.json`, and no `task-034.json`. `task-records` is held by the TASK-013 activation that is running; TASK-001 is not claimed. TASK-031 and TASK-035 hold no lock, so both are dispatchable in parallel. The lock has **six** registered holders — TASK-002, TASK-016, TASK-024, TASK-028, TASK-032, and TASK-034 — and revision 12 adds none, because no architecture task was created. The holder count still only ever grows: TASK-032 completing its authoring did not remove it, which is what makes the count a tally of amendments attempted rather than a measure of contention now.
