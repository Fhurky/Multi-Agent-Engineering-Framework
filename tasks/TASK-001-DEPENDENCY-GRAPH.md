...
**Revision 8**, produced by TASK-013 activation `ACT-007`. One independent verdict, one human governance decision, and one architecture publication drove it, and **no gate is closed by it**:

- **`HUMAN-003` — the TASK-028 execution-provider reroute** — is durable at commit `0b413b7` on the non-agent branch `human/reroute/task-028-gpt`, merged into `main` at `443ff9b` through pull request 16. The user authored the reroute's task-record effects directly. `ACT-007` verified them and **recorded** the decision as ingress entry `seq` 13, class `human_decision_recorded`; it did not re-apply, extend, or reinterpret it. See the `HUMAN-003` section below.
- **TASK-030 round 7** returned `changes-required` on this decomposition at commit `f36e6c06`, with fresh findings **F-601 (High)**, **F-602 (Medium)**, and **F-603 (Low)** — all three Orchestrator-owned — in `reports/code-review/TASK-001-DECOMPOSITION-REVIEW-ROUND-7.md`, published as pull request 14. It recorded F-403 **`not resolved`**; F-401's bootstrap half and the F-301, F-201, and F-104 residuals `partially resolved`; and F-501, F-502, the Orchestrator-owned half of A-209, F-401's schema half, F-402, F-302, F-203, and F-204 `resolved`.
- **TASK-028 published the third architecture amendment** at commit `fe0374c` with pull request 15, satisfying `review_ready(TASK-028)` and releasing TASK-029. **A publication is not a verdict**: no architecture finding is resolved, `gate_passed(LIN-ARCH-REVIEW, review, 4)` is unsatisfied, and nine implementation tasks stay `blocked`.
- **`HUMAN-002` remains approved and unimplemented.** The collector does not exist, TASK-013's declared bootstrap dispatch contract still reads `interim-operator-authorized`, `ACT-007` ran under it, and F-401 is not claimed resolved. TASK-028's published contract representation does not change that; TASK-029 judges it.

The per-finding disposition register is in `tasks/TASK-013-ACTIVATION-LOG.md`, activation `ACT-007`.

Revision history: revision 2 was reviewed by TASK-014 rounds 1 and 2 (`changes-required`, F-001 … F-007 then F-101 … F-105); TASK-015 round 1 returned `changes-required` on the TASK-002 architecture (A-001 … A-004); human governance decision HUMAN-001 was recorded at `fb9f45c`; revision 3 was produced by `ACT-001` and reviewed by TASK-021 round 3 (`changes-required`, F-201 … F-205); revision 4 was produced by `ACT-002` and reviewed by TASK-022 round 4 (`changes-required`, F-301 … F-303); revision 5 was produced by `ACT-004` and reviewed by TASK-023 round 5 (`changes-required`, F-401 … F-403); revision 6 was produced by `ACT-005` and reviewed by TASK-027 round 6 (`changes-required`, F-501 … F-502); revision 7 was produced by `ACT-006` and reviewed by TASK-030 round 7 (`changes-required`, F-601 … F-603). Activation `ACT-003` applied lifecycle updates only and was not a revision.
...
- Invariants 1 and 5: the topological order `HUMAN-001, HUMAN-003, TASK-001, TASK-014, TASK-021, TASK-022, TASK-023, TASK-027, TASK-030, TASK-031, TASK-002, TASK-015, TASK-016, TASK-020, TASK-024, TASK-025, TASK-028, TASK-029, TASK-018, TASK-019, TASK-003, TASK-004, TASK-017, TASK-026, TASK-005, TASK-006, TASK-007, TASK-008, TASK-009, TASK-010, TASK-011, TASK-012, TASK-013` places every task after all of its dependencies and after the pre-merge gate owners of every task it integrates. TASK-012's `gate_passed(LIN-RUNTIME-QA)` edge expands to TASK-011 and points backwards along this order, from position 32 to position 31. The architecture edge held by TASK-003 … TASK-008, TASK-017, TASK-018, and TASK-026 expands, at `lineage_round: 4`, to TASK-029 at position 18, ahead of every one of them. TASK-031 holds only `review_ready(TASK-001)`, already satisfied, and no task holds an edge to TASK-031. No cycle exists.
- Invariant 2: TASK-009, TASK-010, TASK-011, TASK-012, TASK-014, TASK-015, TASK-019, TASK-020, TASK-021, TASK-022, TASK-023, TASK-025, TASK-027, TASK-029, TASK-030, and TASK-031 are the only tasks with `gate_for` entries. Every edge each of them holds to a task it gates is `review_ready`. TASK-012's only non-`review_ready` edge names `LIN-RUNTIME-QA`, of which TASK-012 records no round. TASK-029 records a round of `LIN-ARCH-REVIEW` and holds no `gate_passed` edge at all; TASK-030 and TASK-031 each record a round of `LIN-DECOMP-REVIEW` and hold no `gate_passed` edge at all. None holds an edge naming a lineage it itself records a round of.
- Invariant 3: checked pairwise across all 47 pairs in the gate assignment table and both registers.
...
- Invariant 7: all 47 `gate_for` / `gate_tasks` pairs carry both properties and agree on both across the pair. Each declared `retrospective` was recomputed as `gate ∉ target.pre_merge_gates`: `false` for the **eleven** pairs whose gate is a pre-merge gate — TASK-015 r1, TASK-020 r2, TASK-025 r3, and TASK-029 r4 on TASK-002; TASK-020 r1, TASK-025 r2, and TASK-029 r3 on TASK-016; TASK-025 r1 and TASK-029 r2 on TASK-024; TASK-029 r1 on TASK-028; TASK-019 r1 on TASK-018 — and `true` for the other **36**, which are the 28 runtime and toolchain assembly gates plus the eight rounds of TASK-001's own review gate, since TASK-001 declares `pre_merge_gates: []`. Each declared `gate_class` was recomputed against the publication of the artifact that round reviews: `point` for the **nineteen** decomposition, architecture, and toolchain review rounds, `aggregate` for the 28 assembly gates. 11 + 36 = 47 and 19 + 28 = 47, so both partitions account for every pair.
- Invariant 8: eight lineages are registered. `LIN-DECOMP-REVIEW` declares rounds 1 … 8 with no gap, one task per round except the historical TASK-014 pair at rounds 1 and 2, which is recorded as the single pre-rule exception. `LIN-ARCH-REVIEW` declares rounds 1 … 4, one task each. The remaining six declare round 1 only. Every target named by a pair is a member of its lineage's registered cohort — including TASK-028, which joins the `LIN-ARCH-REVIEW` cohort at round 4 — and every lineage round greater than 1 follows a recorded verdict: `LIN-DECOMP-REVIEW` rounds 2 … 8 follow `changes-required` at `8ac0dbd`, `abb85d9`, `adfb982`, `e8eb23d`, `667d3b8`, `710351fd`, and `f36e6c06`; `LIN-ARCH-REVIEW` rounds 2 … 4 follow `changes-required` at `8632469`, `4874a9d`, and `aa38c7d2`.
...
| TASK-028 | Third architecture amendment: round-3 blocking findings and the approved pre-dispatch observer | architect | gpt | TASK-025 `gate_recorded` | review, published at `fe0374c` |
...
**What the reroute costs, stated rather than absorbed.** TASK-028 and TASK-029 are now both assigned to `gpt`, so the repository's cross-family reviewer preference no longer applies to `LIN-ARCH-REVIEW` round 4. The mandatory guarantee that remains is **execution-context separation**, which `config/agents/settings.yaml` requires unconditionally and which both records state explicitly. That is a narrower margin than the earlier rounds had, and it is recorded as a risk rather than presented as equivalent.
...
**Forty-seven** `gate_for` / `gate_tasks` pairs are declared, stated by enumeration so the total can be recomputed rather than trusted: 8 on TASK-001, 4 on TASK-002, 3 on TASK-016, 2 on TASK-024, 1 on TASK-028, 3 each on TASK-003, TASK-004, TASK-007, TASK-017, and TASK-026, 4 each on TASK-005, TASK-006, and TASK-008, and 2 on TASK-018 — that is 8 + 4 + 3 + 2 + 1 + 15 + 12 + 2. Revision 7 added five: one decomposition round and the four relations TASK-029 carries. Revision 8 adds exactly one, the `LIN-DECOMP-REVIEW` round-8 relation TASK-031 carries. Each pair's `gate_class`, `retrospective`, `gate_lineage`, and `lineage_round` are declared in the pair's own frontmatter and summarized in the registers above; they are not restated here or in any record body.
...
2. **A superseding round is a new task, not a re-entrant one, and the relation that survives supersession is the lineage.** TASK-014, TASK-015, TASK-020, TASK-021, TASK-022, TASK-023, TASK-025, TASK-027, and TASK-030 recorded durable verdicts and are `done`. TASK-029 and TASK-031 perform the next rounds.

## Execution waves

```text
Wave 0   TASK-031                            decomposition re-review round 8
         TASK-013                            event-triggered, quiescent, not a wave member
Wave 1   TASK-029                            gate the third amendment before it merges, ready now
         TASK-028                            third architecture amendment, published at fe0374c, awaiting its gate
...
Revision 5 inserted TASK-026 at Wave 4 and moved the scheduler and everything after it one wave later, because the scheduler's ingress observer now reads a durable inbox that TASK-026 owns. Revision 7 changed only Wave 0 and Wave 1 membership: TASK-025 and TASK-027 recorded their verdicts and left, TASK-030 replaced TASK-027 at Wave 0, and TASK-028 and TASK-029 replaced TASK-025 at Wave 1. Revision 8 changes the same two: TASK-030 recorded its verdict and left, TASK-031 replaces it at Wave 0, and TASK-028 has published so **TASK-029 is the dispatchable half of Wave 1** while TASK-028 waits on its own gate. **Wave 1 is in its third attempt** — amend, gate, fail, amend again — and every later wave has moved with it. That is the cost the aggregate register records for the architecture lineage, and it is stated rather than absorbed silently into the wave numbering. TASK-013 is not a wave and is not claimed at wave boundaries on a timer. It is dispatched only when an unconsumed ingress fact exists; see the activation model below.
...
The architecture at commit `9576fc9` on `agent/claude/architect/task-002` was the normative technical source. TASK-015 round 1 returned `changes-required` with A-001 … A-004; TASK-016 amended it at `8d0c570`; TASK-020 returned `changes-required` at `4874a9d` with A-101 … A-105, judging A-001 `resolved`, A-002 and A-003 `partially resolved`, and A-004 `not resolved`; TASK-024 amended it at `c2ee3eb`; **TASK-025 returned `changes-required` at `aa38c7d2`** with A-201 … A-209, judging A-101, A-102, A-104, and A-004 `not resolved` and A-103, A-105, A-002, and A-003 `partially resolved`. **TASK-028 has now published the next amendment at `fe0374c`, and no verdict on it exists.**

**No approved architecture source exists.** Every round of this lineage has returned `changes-required`: round 1 rejected `9576fc9`, round 2 rejected `8d0c570`, and round 3 rejected `c2ee3eb`. **The architecture this graph will build on is `9576fc9` as amended by `8d0c570`, by `c2ee3eb`, and further amended by the TASK-028 commit that TASK-029 approves** — and it comes into being only when `LIN-ARCH-REVIEW` records a passing or formally accepted authoritative verdict at `lineage_round` 4 or later. Until then the graph names a *target*, not an approved source. TASK-028's publication does not change that: `fe0374c` is `review_ready`, not approved. Every architecture dependency edge names the lineage `LIN-ARCH-REVIEW` at `lineage_round: 4` rather than any single task, so no further retarget is required if a fifth round becomes necessary.

**Normative source rule.** F-202 recorded that TASK-003 still pinned the rejected baseline alone; **F-601 then recorded that the nine consumers named the round-3 amendment as one TASK-025 approves, when TASK-025 had rejected it.** Every implementation record now states its normative source in the same corrected form: *the named documents at `9576fc9` **as amended by `8d0c570`, by `c2ee3eb`, and by the TASK-028 commit that TASK-029 approves***, together with the explicit statement that none of the three named baselines is approved. A record that cites a baseline without the amendment clause is a finding; a record that names `8d0c570`, `c2ee3eb`, or `fe0374c` as approved is a finding; and a record that attributes an approval to a round that recorded `changes-required` is a finding. No superseded baseline and no ungated amendment is ever the normative source on its own. **A raised lineage floor is not by itself sufficient** — F-601 recorded that the floor was mechanically correct at round 4 while the source clause still pointed at the rejected round-3 artifact, so the floor and the source clause must be moved together.

**The F-401 contract defect was present in the round-3 amendment and is not decided here.** `c2ee3eb` declared `consumedBy: ActivationId | null` on the inbox entry at `docs/architecture/runtime/INTERFACE-CONTRACTS.md:551` and repeated the single-schema model in ADR-0017 and `STATE-MACHINE.md`. **TASK-025 judged it and recorded four of the six Part D checks `not satisfied` at `aa38c7d2`**, so the defect stood at the reviewed commit. It is routed to TASK-028 as scope item 1 and item 9, and TASK-028's owner records at `fe0374c` that the amendment defines immutable ingress entries and named bootstrap dispatch contracts. **Whether it actually does is TASK-029's judgment, not this graph's**: the Orchestrator records the owner's claim as a claim, neither pre-judging that verdict nor claiming F-401 resolved by architecture.

**Module map.** Each of the six modules in `docs/architecture/runtime/COMPONENT-BOUNDARIES.md` maps to exactly the owner task this graph assigns, and the source paths match the write-scope partition below. The seventh module added by TASK-016 maps to TASK-017. The **eighth** module, the durable ingress inbox, maps to TASK-026; TASK-024's commit message stated it was added at `c2ee3eb`, and TASK-025 checked that claim rather than the graph accepting it. TASK-028's owner records at `fe0374c` that the eight-module acyclic map and its two independent contract roots are preserved; **TASK-029 checks that claim**, and the graph does not accept it.
...
| No module owns the agent workspace lifecycle that `AgentInvocation.worktreePath` presupposes | TASK-016, then TASK-024, then TASK-028, then TASK-017 | TASK-016 published `WORKSPACE-LIFECYCLE.md` and ADR-0011 at `8d0c570`. TASK-020 judged the module ownership documented but not implementation-ready: A-102 and A-103 record that registration and workspace intent cannot be made durable before their side effects. TASK-024 published its remediation at `c2ee3eb` and **TASK-025 judged it `changes-required` at `aa38c7d2`**, leaving A-102 and A-103 unresolved through A-205 and A-206. TASK-028 published the next remediation at `fe0374c`; **TASK-029 decides whether it is resolved** |
| No module owns the durable ingress inbox the activation model requires | TASK-024, then TASK-028, then TASK-026 | Opened by F-301. TASK-024 published a module and contract at `c2ee3eb`; **TASK-025 judged ten of eleven Part B checks satisfied and four of six Part D checks not satisfied**, so the contract half is unresolved. TASK-028 published its remediation at `fe0374c`; **TASK-029 decides whether it satisfies the Part D correction F-401 requires**. TASK-026 implements it once an approved contract exists |
| **No authorized producer can append a durable ingress entry before TASK-013 is selected during bootstrap** | **HUMAN-002** decided it; **TASK-028** for the contract, **TASK-026** for the collector, **TASK-005** for the signal, TASK-018 for any CI tooling | **Decided, contract published, still open as a capability.** Opened by F-401. `HUMAN-002` was approved on 2026-08-05 and assigns the append to a Runtime-owned durable pre-dispatch collector outside `tasks/**`, so the ownership gap has an owner and a shape — and an Orchestrator-authored append is still excluded, which is what keeps F-201's self-trigger closed. TASK-028 published a contract representation at `fe0374c`, including ADR-0031 `pre-dispatch-ingress-observer-and-collector` and ADR-0023 `immutable-ingress-entries-and-named-bootstrap-dispatch-contracts`; **that is a publication, not an approval, and TASK-029 judges it.** The gap itself remains until the collector is implemented and independently validated; TASK-013 runs under the declared `interim-operator-authorized` dispatch contract until then — `ACT-007` ran under it — which the graph names as a limitation rather than presenting as the durable predicate |
...
The thirteen reviewer-owned report files — TASK-009's two paths, TASK-014, TASK-015, TASK-019, TASK-020, TASK-021, TASK-022, TASK-023, TASK-025, TASK-027, TASK-029, TASK-030, and TASK-031 — are mutually path-disjoint by construction, so any of them may run concurrently.
...
At revision 8, `architecture-docs` is **free**: the TASK-028 execution completed and released it. `task-records` is held by the TASK-013 activation that is running; TASK-001 is not claimed. TASK-029 and TASK-031 hold no lock and are unaffected, so both are dispatchable in parallel and neither can collide with the other or with a future `architecture-docs` holder. The `ACT-007` activation read the lock directory directly rather than inferring the state from the earlier record: it contains exactly one entry, `task-013.json`, naming this worktree and session. The lock now has four registered holders — TASK-002, TASK-016, TASK-024, and TASK-028 — and revision 8 adds none.
...
| Validating owner (TASK-009 … TASK-012, TASK-014, TASK-015, TASK-019 … TASK-023, TASK-025, TASK-027, TASK-029, TASK-030, TASK-031) | Its own report scope; the pull request description | Any file under `tasks/` |
...
TASK-014 / TASK-015 / TASK-019 … TASK-023, TASK-025, TASK-027, TASK-029 … TASK-031   decomposition, architecture, and toolchain findings
