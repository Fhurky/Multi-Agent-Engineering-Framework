...
**Revision 14**, produced by TASK-013 activation `ACT-013`. **No verdict drove it and no gate is closed by it.** It consumed one `artifact_published` fact:
...
- Invariant 3: checked pairwise across all 65 pairs in the gate assignment table and both registers.
...
| TASK-003 | Durable run state and checkpoints | runtime | claude | `LIN-ARCH-REVIEW` `gate_passed(review, 7)`; TASK-018 `integrated` | blocked |
| TASK-004 | Provider adapters and agent workers | runtime | claude | `LIN-ARCH-REVIEW` `gate_passed(review, 7)`; TASK-018 `integrated` | blocked |
| TASK-005 | Scheduling, leases, fencing, bounded concurrency | runtime | claude | `LIN-ARCH-REVIEW` `gate_passed(review, 7)`; TASK-003, TASK-004, TASK-026 `integrated` | blocked |
| TASK-006 | Supervisor core and state machine | runtime | claude | `LIN-ARCH-REVIEW` `gate_passed(review, 7)`; TASK-003, TASK-004, TASK-005, TASK-017 `integrated` | blocked |
| TASK-007 | Lifecycle control and one-input bootstrap | runtime | claude | `LIN-ARCH-REVIEW` `gate_passed(review, 7)`; TASK-006 `integrated` | blocked |
| TASK-008 | Crash recovery, timeouts, idempotent retries | runtime | claude | `LIN-ARCH-REVIEW` `gate_passed(review, 7)`; TASK-003, TASK-004, TASK-006, TASK-017 `integrated` | blocked |
...
| TASK-017 | Agent workspace lifecycle automation | runtime | claude | `LIN-ARCH-REVIEW` `gate_passed(review, 7)`; TASK-003, TASK-018 `integrated` | blocked |
| TASK-018 | Runtime toolchain bootstrap | devops | claude | `LIN-ARCH-REVIEW` `gate_passed(review, 7)` | blocked |
...
| TASK-026 | Durable ingress inbox and activation cursor store | runtime | claude | `LIN-ARCH-REVIEW` `gate_passed(review, 7)`; TASK-003, TASK-018 `integrated` | blocked |
...
| TASK-036 | Sixth architecture amendment: the round-6 blocking findings | architect | gpt | TASK-035 `gate_recorded` | review, published `local-only` at `970b081`, unjudged |
| TASK-037 | Independent review of the sixth architecture amendment | reviewer | gpt | TASK-036 `review_ready` | ready |
...
**Sixty-five** `gate_for` / `gate_tasks` pairs are declared, stated by enumeration so the total can be recomputed rather than trusted: 8 on TASK-001, 7 on TASK-002, 6 on TASK-016, 5 on TASK-024, 4 on TASK-028, 3 on TASK-032, 2 on TASK-034, 1 on TASK-036, 3 each on TASK-003, TASK-004, TASK-007, TASK-017, and TASK-026, 4 each on TASK-005, TASK-006, and TASK-008, and 2 on TASK-018 — that is 8 + 7 + 6 + 5 + 4 + 3 + 2 + 1 + 15 + 12 + 2. Revision 7 added five: one decomposition round and the four relations TASK-029 carried. Revision 8 added exactly one, the `LIN-DECOMP-REVIEW` round-8 relation TASK-031 carries. Revision 9 added **five**, the `LIN-ARCH-REVIEW` round-5 relations TASK-033 carries. Revision 10 added none, because activation `ACT-009` created no task and no relation. Revision 11 added **six**, the `LIN-ARCH-REVIEW` round-6 relations TASK-035 carries, and enriched the five round-5 relations with their recorded verdict. Revision 12 added none. Revision 13 added **seven**, the `LIN-ARCH-REVIEW` round-7 relations TASK-037 carries, and enriched the six round-6 relations with their recorded verdict. **Revision 14 adds none**, because activation `ACT-013` created no task and no relation; the seven round-7 relations it inherited became dispatchable and all seven remain `pending` on both sides of their pairs. The round-6 cohort is the largest this graph has declared, which is a direct measure of how many times this lineage has been amended without passing. Each pair's `gate_class`, `retrospective`, `gate_lineage`, and `lineage_round` are declared in the pair's own frontmatter and summarized in the registers above; they are not restated here or in any record body.
...
Wave 1   TASK-037                            gate the sixth amendment before it merges, ready now
         TASK-036                            sixth architecture amendment, published local-only at 970b081, review, unjudged, not integrable
...
**No approved architecture source exists.** Every round of this lineage has returned `changes-required`: rounds 1 … 6 rejected `9576fc9`, `8d0c570`, `c2ee3eb`, `fe0374c`, `468b37b`, and `6d145eb`. **The architecture this graph will build on is `9576fc9` as amended by `8d0c570`, by `c2ee3eb`, by `fe0374c`, by `468b37b`, by `6d145eb`, and further amended by the TASK-036 commit that TASK-037 approves** — and it comes into being only when `LIN-ARCH-REVIEW` records a passing or formally accepted authoritative verdict at `lineage_round` 7 or later. Until then the graph names a *target*, not an approved source. Every architecture dependency edge names the lineage `LIN-ARCH-REVIEW` at `lineage_round: 7` rather than any single task, so no further retarget is required if an eighth round becomes necessary — only the floor moves, and under F-601 it never moves without the source clause moving in the same commit.

**Normative source rule.** F-202 recorded that TASK-003 still pinned the rejected baseline alone; **F-601 then recorded that the nine consumers named the round-3 amendment as one TASK-025 approves, when TASK-025 had rejected it.** Every implementation record now states its normative source in the same corrected form: *the named documents at `9576fc9` **as amended by `8d0c570`, by `c2ee3eb`, by `fe0374c`, by `468b37b`, by `6d145eb`, and by the TASK-036 commit that TASK-037 approves***, together with the explicit statement that none of the six named baselines is approved. A record that cites a baseline without the amendment clause is a finding; a record that names `8d0c570`, `c2ee3eb`, `fe0374c`, `468b37b`, or `6d145eb` as approved is a finding; and a record that attributes an approval to a round that recorded `changes-required` is a finding. No superseded baseline and no ungated amendment is ever the normative source on its own. **A raised lineage floor is not by itself sufficient** — F-601 recorded that the floor was mechanically correct at round 4 while the source clause still pointed at the rejected round-3 artifact, so the floor and the source clause must be moved together.
...
The sixteen reviewer-owned report files — TASK-009's two paths, TASK-014, TASK-015, TASK-019, TASK-020, TASK-021, TASK-022, TASK-023, TASK-025, TASK-027, TASK-029, TASK-030, TASK-031, TASK-033, TASK-035, and TASK-037 — are mutually path-disjoint by construction, so any of them may run concurrently.
...
At revision 14, `architecture-docs` is **free**: TASK-036's execution released its lock after its commit, confirmed by a direct read of the shared lock directory at `ACT-013`, which holds exactly one entry, `task-013.json`, and no `task-036.json`. `task-records` is held by the TASK-013 activation that is running; TASK-001 is not claimed. TASK-031 and TASK-037 hold no lock, so both are dispatchable in parallel. The lock has **seven** registered holders — TASK-002, TASK-016, TASK-024, TASK-028, TASK-032, TASK-034, and TASK-036 — and revision 14 adds none, because no architecture task was created. The holder count still only ever grows: TASK-032 completing its authoring did not remove it, which is what makes the count a tally of amendments attempted rather than a measure of contention now.
