...
**Revision 9**, produced by TASK-013 activation `ACT-008`. **One** independent verdict drove it, and **no gate is closed by it**:
...
Wave 1   TASK-032                            fourth architecture amendment, ready now
         TASK-033                            gate the fourth amendment before it merges, blocked until TASK-032 publishes
...
Revision 5 inserted TASK-026 at Wave 4 and moved the scheduler and everything after it one wave later, because the scheduler's ingress observer now reads a durable inbox that TASK-026 owns. Revision 7 changed only Wave 0 and Wave 1 membership: TASK-025 and TASK-027 recorded their verdicts and left, TASK-030 replaced TASK-027 at Wave 0, and TASK-028 and TASK-029 replaced TASK-025 at Wave 1. Revision 8 changed the same two. **Revision 9 changes only Wave 1**: TASK-029 recorded its verdict and left, and TASK-032 and TASK-033 join, with **TASK-032 the dispatchable half of Wave 1**. TASK-031 is unchanged at Wave 0 and remains dispatchable in parallel. **Wave 1 is now in its fourth attempt** — amend, gate, fail, amend again, four times over — and every later wave has moved with it each time. That is the cost the aggregate register records for the architecture lineage, and it is stated rather than absorbed silently into the wave numbering. TASK-013 is not a wave and is not claimed at wave boundaries on a timer. It is dispatched only when an unconsumed ingress fact exists; see the activation model below.
...
At revision 9, `architecture-docs` is **free**: the TASK-028 execution completed and released it, and TASK-029 declared no lock. `task-records` is held by the TASK-013 activation that is running; TASK-001 is not claimed. TASK-031 and TASK-033 hold no lock and are unaffected, so TASK-031 and TASK-032 are dispatchable in parallel and neither can collide with the other. The `ACT-008` activation read the lock directory directly rather than inferring the state from the earlier record: it contains exactly one entry, `task-013.json`, naming this worktree and session. The lock now has **five** registered holders — TASK-002, TASK-016, TASK-024, TASK-028, and TASK-032 — and revision 9 adds exactly one, TASK-032, which is not yet claimed.
