...
integrable_reason: >-
  ...
  pull requests 22, 25, and 28 unmerged and unmodified. THAT MERGE HAPPENED AT ACT-027, at
  cf6333b10e628b3b61f3b7f8716b30923725067d, and ACT-027 consumed the fact rather than producing
  it.
integrated: true
integrated_at: cf6333b10e628b3b61f3b7f8716b30923725067d
integrated_evidence: >-
  LINEAGE-SUBSUMED, not content-merged. This record receives NO COMMIT OF ITS OWN and contributes
  no Git content of its own to the integration branch, which is what ADR-0041 and the approved
  INTEGRATION-STRATEGY.md prescribe for a superseded cohort member. The integration branch is
  integration/autonomous-runtime; the merged commit is cf6333b10e628b3b61f3b7f8716b30923725067d;
  the source task T is TASK-046; the source published commit is
  f148567d716c00d7a24783318c8d6d7031492e7b; the lineage is LIN-INTEGRATION-AUTHORITY-REVIEW; the
  authoritative round is lineage_round 3. Every one of those six values is identical to the value
  on TASK-046's and TASK-040's records, which is the batch-consistency property the strategy
  requires. THE ANCESTRY WAS PROVED RATHER THAN ASSERTED - git merge-base --is-ancestor
  e33a62beb8198162db7c37f4e9740269e1454d2d cf6333b10e628b3b61f3b7f8716b30923725067d succeeds, and
  e33a62be is the LITERAL PARENT of f148567d, read with git cat-file -p rather than inferred. The
  chain is 5e5fc8f then e33a62be then f148567d, three commits on one branch, and the merge landed
  the whole chain in one commit. GITHUB CLOSED PULL REQUEST 25 AS MERGED AT 2026-08-07T21:06:08Z
  with mergeCommit.oid equal to e33a62beb8198162db7c37f4e9740269e1454d2d, which is this record's
  OWN HEAD and not a separate merge commit - read live from the API at ACT-027 and recorded as
  external corroboration that no second merge exists, not as a second integration. LINEAGE
  SUBSUMPTION IS LIFECYCLE EVIDENCE AND NOT A GIT OPERATION, and it is not a claim that this
  record's own publication was approved in isolation - it says that this record's historical
  responsibility is contained in the one reviewed cumulative tree. Its round-1 changes-required
  verdict at 6f7f0edb63615d7f143dd6c59750a5ea7db701fc remains durable and unrewritten, and its
  round-2 approved verdict at 78359ae2e3dc6e97fb3d60f0b847b84abed08fa6 is what closed its relation.
  Because the same authoritative verdict already closed every cohort relation, this record may
  satisfy terminal and reach done without replaying its blobs, which INTEGRATION-STRATEGY.md states
  in those terms. The source branch agent/gpt/architect/task-042 was NOT deleted and still resolves
  to e33a62be both locally and on origin.
round_1_verdict: >-
...
integration_ancestry:
  ...
  known_divergence: RESOLVED AT ACT-027 IN THE ONLY WAY IT COULD BE. This record's published head e33a62beb8198162db7c37f4e9740269e1454d2d is now an ancestor of integration/autonomous-runtime, which resolves to cf6333b10e628b3b61f3b7f8716b30923725067d - the two lines converged when the operator merged pull request 28, rather than the branch point moving. git merge-base --is-ancestor e33a62be cf6333b succeeds. Nothing about this task's branch point, base, scope, findings, gate, rounds, or acceptance criteria changed at ACT-027; only this currentness statement did, and it is ordinary revision work superseded by an external event rather than a model correction. The superseded value read - This branch point does not contain 461511437a26a57fe9a976c5ce3222ca123084d1, the then-current head of integration/autonomous-runtime. That commit is a one-parent operator synchronization whose only changed paths are under tasks/**, which is outside this role's write scope and outside the reviewed delta. The two lines diverge only between tasks/** and docs/**. This is recorded so that no reader infers an ancestry that does not exist and no owner treats the difference as a conflict to resolve.
...
- **Transition at `ACT-027`: `review` → `done`, on ingress entry `seq` 37, class `branch_integrated`, LINEAGE-SUBSUMED.** The operator merged **pull request 28** — TASK-046's, not this record's — into `integration/autonomous-runtime` at **`cf6333b10e628b3b61f3b7f8716b30923725067d`**. **This record's own branch was not merged and receives no commit of its own**, which is what the approved integration strategy prescribes for a superseded cohort member and is not an omission. Its published head `e33a62beb8198162db7c37f4e9740269e1454d2d` is `f148567d`'s literal parent and therefore an ancestor of the merge, proved with `git merge-base --is-ancestor` and `git cat-file -p` rather than inferred. **GitHub closed pull request 25 as `MERGED` with `mergeCommit.oid` equal to this record's own head OID**, read live and recorded as corroboration of the ancestry rather than as a second integration. Its three `integrated` clauses are each satisfied: `review_ready` at `e33a62be`, its single `pre_merge_gates` entry `review` closed at `approved` at round 2, and the content merged into the integration branch as part of the one cumulative unit. **`ACT-027` merged nothing and simulated nothing.** See `integrated_evidence`.
- **What reaching `done` does not mean here, stated because subsumption is the easiest thing in this graph to over-read.** It does **not** mean this record's own publication was approved in isolation — round 1 recorded `changes-required` at `6f7f0edb` and that verdict is durable and unrewritten. It does **not** mean anything was re-reviewed at `ACT-027`. It means only that the one reviewed cumulative tree contains this record's content and that the same authoritative verdict already closed this record's relation, so `terminal` is satisfied without replaying its blobs.
