...
review_target_commit: pending; bind to the exact TASK-062 runtime publication before dispatch
...
blocked_reason: >-
  review_ready(TASK-062) is unsatisfied because TASK-062 has not published an immutable runtime-class
  target. This record is not dispatchable and carries no verdict.
exit_condition: >-
  TASK-013 independently verifies TASK-062's immutable published commit, exact origin ref, open or
  updated non-draft pull request with exact head, and relevant evidence, then binds that commit and
  changes only this scheduling edge from unsatisfied to satisfied.
