...
supersedes: TASK-029
blocked_reason: TASK-032 has not published the fourth architecture amendment, so there is no immutable target to review. The declared dependency review_ready(TASK-032) is unsatisfied.
exit_condition: TASK-032 reaches review_ready — an immutable published commit on agent/gpt/architect/task-032 whose publication_class bootstrap is satisfied. No merge is required. The Orchestrator then binds review_target_commit from the branch as published and moves this record to ready.
...
review_target_applicability: applicable and resolved for the base; the target commit is a reproducible value the Orchestrator pins at the activation that consumes TASK-032's publication
review_target_note: The target is TASK-032's immutable published commit, compared against review-diff base fe0374c, the TASK-028 amendment it revises. Read c2ee3eb, 8d0c570, and 9576fc9 where a judgment needs an earlier baseline. The target commit is deliberately not guessed here; the Orchestrator reads it from the branch as published, as ACT-007 did for fe0374c and ACT-005 for c2ee3eb. Once bound it is immutable and no later activation changes it.
...
Branch `agent/gpt/architect/task-032`, at the immutable published commit that TASK-013 binds into `review_target_commit` from the branch as published, compared against **`fe0374c`** — the TASK-028 amendment this one revises. Read `c2ee3eb`, `8d0c570`, and `9576fc9` where a judgment needs an earlier baseline.

**Read the target as three separate sets, and say which is which in the report.**

- the **authored delta**, what TASK-032's owner wrote, against that branch's own immutable branch point;
...
- the **ancestry difference**, any path outside `docs/` and `diagrams/` that appears in an unrestricted diff because of where the branch was rooted. **None of it may be attributed to TASK-032.** Finding F-502 recorded what conflating a target diff with a review scope costs; finding A-209 recorded what attributing inherited paths to the wrong owner costs.

TASK-032's record instructs its owner to branch from a commit that already contains `8d0c570`, `c2ee3eb`, and `fe0374c`, so the ancestry gap that made pull request 15 `CONFLICTING` should not recur. **Verify that from the repository rather than assuming it**, and report the integration state you find, including whether any architecture document present at `fe0374c` is absent at the target.

**No document in this lineage has ever recorded a passing verdict.** `9576fc9`, `8d0c570`, `c2ee3eb`, and `fe0374c` are each superseded authoring baselines, not approved sources. Report any statement in the target that treats one of them as approved, and any statement that attributes an approval to a round that recorded `changes-required` — finding F-601 recorded that defect in the decomposition and the same standard applies to the contracts.
...
| A-301 | The evidence anchor resolves. Re-run a full relative-link and heading-fragment check and report both the count checked and the count failed. |
...
- Verify that every change is an amendment naming what it supersedes, that no existing contract or ADR decision body is silently rewritten, and that every ADR whose decision changed carries an explicit supersession record.
...
- `reports/code-review/TASK-032-ARCHITECTURE-AMENDMENT-REVIEW-ROUND-4.md` containing the coverage statement, the Part A dispositions, the Part B judgments on the four inherited views, the six Part C re-assessments, the fresh Part D findings, the assessment of every TASK-032 acceptance criterion, the single verdict, and the statement of its atomic application to all five gate relations.

## Write-scope isolation

This task's single file is new and path-disjoint from TASK-009's `reports/code-review/REVIEW.md` and `reports/code-review/runtime/**`, and from the single files of TASK-014, TASK-015, TASK-019, TASK-020, TASK-021, TASK-022, TASK-023, TASK-025, TASK-027, TASK-029, TASK-030, and TASK-031. No resource lock is required, so this task may run concurrently with any other reviewer-owned task.

## Gate and remediation path

This task records five gate relations as `gate_for` reverse edges rather than scheduling dependencies. Their gate names, rounds, scheduling classes, ordering against integration, lineage, and lineage round are declared in the frontmatter above and summarized in the registers in `tasks/TASK-001-DEPENDENCY-GRAPH.md`; this body does not restate them. It becomes dispatchable when TASK-032 reaches `review_ready` — an immutable published commit, no merge required. TASK-032 becomes integrable only after this task's verdict closes its review gate.
...
3. Read the target with `git diff fe0374c..<review_target_commit> -- docs diagrams` for the cumulative amendment and `git diff <target parent>..<review_target_commit>` for the authored delta. Read all four baseline reports and `tasks/TASK-013-ACTIVATION-LOG.md` model correction `MC-006` before judging Part C.
4. Before handoff, resolve the immutable branch point with `git merge-base HEAD agent/claude/orchestrator/task-013` and run `scripts/orchestration/validate-write-scope.ps1 -IncludeWorkingTree -BaseRef <that value>`. Do not pass a review-diff base, `fe0374c`, `c2ee3eb`, `8d0c570`, `c325275`, or `origin/main`.
