...
- Implement the consumption model findings F-201 and F-301 require: the per-task cursor is the **only** representation of consumption state, the consumption ledger is append-only and is written by the consuming activation, the observer never mutates a ledger row and never treats a ledger field as the dispatch signal, and the observer rejects at load time a cursor greater than `ingress_seq` or a cursor that would decrease.
...
- `integrated(TASK-026)` supplies the durable ingress inbox this task's observer reads. TASK-026 owns the store, the `seq` assignment, the `fact_id` and `content_hash` computation, the adapters, the class precedence, and the self-exclusion rule; this task owns the observer, the dispatch predicate, the cursor, and the one-commit rule over it. The two write scopes are disjoint.
