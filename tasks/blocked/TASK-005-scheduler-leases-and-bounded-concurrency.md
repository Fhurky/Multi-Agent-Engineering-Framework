...
- Consume the **high-water-mark signal from the durable pre-dispatch ingress collector** that human governance decision `HUMAN-002` approved on 2026-08-05, through the interface the approved TASK-028 contract declares. `HUMAN-002` is transcribed verbatim in `tasks/TASK-013-ACTIVATION-LOG.md` as model correction `MC-006`; read it there. The collector is **runtime-owned**, lives outside `tasks/**` in the runtime control plane, and — before this module selects the recurring task — validates and deduplicates an external source fact and appends the immutable entry through the **TASK-026** ingress store, then exposes or signals the new high-water mark to this module. **This module does not append.** Its half of the seam is reading or receiving that mark, evaluating the predicate against the cursor, and dispatching; the append path, its deduplication, and its authorized-appender check are TASK-026's. The two write scopes stay disjoint, and neither module may write under `tasks/`.
- Treat the approval as a routing fact, not a capability. Until the collector is implemented and independently validated, a recurring task declaring `interim-operator-authorized` must still surface the unmet precondition. This module must not report the durable predicate as satisfied because a governance decision exists.
