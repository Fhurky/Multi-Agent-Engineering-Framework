...
review_target_applicability: applicable per gated relation rather than once. Each of TASK-003 through TASK-008, TASK-017, and TASK-026 declares its own review_target_base, and this task validates each against that value. Until then there is no single base to record, and recording one would be an assertion rather than a reading.
...
- [ ] **`V11-F401-PREDISPATCH` — a durable entry exists before dispatch, and consumption never mutates it.** One end-to-end test that constructs the operational deadlock finding F-401 described. Under the `durable-bootstrap-append` contract: an authorized appender commits an entry outside `tasks/**` **before** the recurring task is selected; the observer then sees `ingress_seq > cursor` and dispatches; the activation's effects, its ledger rows, and its cursor advance land in one commit; and the task returns to quiescence. The test must additionally assert that the inbox entry is **byte-identical** before and after consumption, that the ledger row is a separate record referencing it, and that a run in which the activation itself creates the only durable evidence is **rejected** rather than dispatched. A second case asserts that a recurring task declaring `interim-operator-authorized` surfaces the unmet precondition instead of reporting the durable predicate as satisfied. A third case exercises the end-to-end loop through the collector that human governance decision `HUMAN-002` approved on 2026-08-05, transcribed as `MC-006` in `tasks/TASK-013-ACTIVATION-LOG.md`: an external source fact is validated and deduplicated by the runtime-owned collector, appended through TASK-026's store, and its new high-water mark signalled to TASK-005 — all **before** the scheduler selects the recurring task — after which the activation consumes, commits its effects and cursor in one commit, and returns to quiescence. The test must assert that re-presenting the same source fact appends nothing, and that the durable contract becomes satisfiable only once this path runs, so that the governance approval alone never flips the declared contract.
