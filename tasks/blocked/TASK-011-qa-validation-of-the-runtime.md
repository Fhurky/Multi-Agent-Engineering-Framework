...
**This gate is `aggregate` and `retrospective` for every target**, declared on both sides of each pair and registered with a reason and a recorded risk in `tasks/TASK-001-DEPENDENCY-GRAPH.md`, row "TASK-011 / qa". End-to-end lifecycle validation requires a startable runtime, so this gate cannot be split per target; the cost is that a defect in an early module surfaces only after every later module was built on it.

**This task's verdict is a scheduling precondition for TASK-012.** TASK-012 holds `gate_passed(TASK-011, qa, 1)`, the owner form of the edge, which is satisfied only by a **passing** verdict. A `changes-required` verdict here leaves TASK-012 blocked until a later QA round passes; it does not release performance validation against a failed baseline. That is the correction finding F-204 required.
