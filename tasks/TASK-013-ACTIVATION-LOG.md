...
| 2 | Durable append-only ingress inbox; `ingress_seq = max(seq)`; identity by `fact_id`; positions assigned once at append | `seq_base = 6`, entries from `seq` 7; currently `seq` 7 … 12 | **Active** |
...
`ingress_seq = 12`. `activation.last_consumed_event_seq = 12`. TASK-013 is quiescent.
...
- Effects commit hash: recorded by the follow-up commit. It is TASK-030's immutable review target, against review-diff base `62d6f2d`.
