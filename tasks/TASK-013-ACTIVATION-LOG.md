...
| 2 | Durable append-only ingress inbox; `ingress_seq = max(seq)`; identity by `fact_id`; positions assigned once at append | `seq_base = 6`, entries from `seq` 7 | **Active** |
...
`ingress_seq = 10`. `activation.last_consumed_event_seq = 10`. TASK-013 is quiescent.
...
**Append-only audit.** Rows 1 … 8 are byte-identical to their prior state. Rows 9 and 10 were appended; nothing was substituted, no row was renumbered, no epoch boundary was crossed, and no `consumed_by` value was mutated. `MC-004` changed the normative **entry** schema and left every row here untouched.
...
- Effects commit hash: **PENDING** — recorded by the follow-up commit. It is TASK-027's immutable review target, against review-diff base `890b8e0`.
