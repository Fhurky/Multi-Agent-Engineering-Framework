...
Normative retry and idempotency contract for the autonomous runtime. Produced under TASK-002 and amended under TASK-016, TASK-024, and TASK-028. Related decisions: [ADR-0006](../../adr/0006-retry-classification-backoff-and-idempotency-keys.md), [ADR-0013](../../adr/0013-single-decision-recovery-reconciliation.md), [ADR-0014](../../adr/0014-live-run-control-and-process-tree-ownership.md), [ADR-0020](../../adr/0020-durable-adoptable-results-for-recovery.md), and [ADR-0025](../../adr/0025-result-effect-identity-in-the-event-union.md). Implemented by TASK-008 against the taxonomy owned by TASK-004.
...
`EffectIntentRecorded` is the event that sets `isTaskResultEffect`; the transition copies that value into the ledger entry. Exactly one effect per `(taskId, attempt)` may set it to `true`, and it is legal only after the matching `WorkerResultRecorded`. A second flagged intent, an unflagged intent later treated as the result effect, or `EffectCommitted` before the flagged intent is rejected. Recovery does not replay steps 0 through 3 for an adopted result; it reads those already-durable facts and emits only step 4.
