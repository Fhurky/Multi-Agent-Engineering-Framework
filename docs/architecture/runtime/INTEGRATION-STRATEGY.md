...
TASK-040 adds the two conditional executors authorized by HUMAN-004 at `7dc07488a5b1cac8b1327ebd63bf747adbe03c68`. ADR-0042 supersedes only the operator-execution clauses: all typed order, squash, cumulative-unit, tree-equality, and contract-change rules remain. TASK-041 recorded `changes-required` at `ec533fb5bb0055675fb81f72057d5636f7867db3`. TASK-042 resolves F-041-01 with exact executor-local gate admissibility and a returned tasks-owned graph narrowing, resolves F-041-02 with a separate signed current-policy boundary plus an explicit unresolved GitHub control-plane dependency, and resolves F-041-04 with exact published-head owner evidence. ADR-0043 owns those corrections. This cumulative amendment records no approval; TASK-044 alone owns the next verdict.
...
**Executor, identity, and current policy.** After its activation conditions hold, the runtime post-gate executor performs Steps 1 through 8 one admitted content unit at a time using `squash`. The DevOps release executor performs Step 9 using the protected PR `merge` method. Each plan pins the exact PR head and base OIDs, the next `IntegrationOrderKey`, and a complete fresh plan-bound policy attestation from the separate human-controlled policy plane; a mutable branch name only selects a ref for an immediate equality check. A base change, stale head, wrong method, earlier missing unit, order-key mismatch, incomplete bypass set, signature/freshness/revocation failure, or policy drift refuses before mutation.
...
TASK-016, TASK-024, TASK-028, TASK-032, TASK-034, TASK-036, TASK-038, TASK-040, and TASK-042 each apply the procedure below once. TASK-015, TASK-020, TASK-025, TASK-029, TASK-033, TASK-035, and TASK-037 independently rejected preceding authoring baselines; TASK-039 approved TASK-038 at round 8; TASK-041 recorded `changes-required` for TASK-040. TASK-044 alone may judge the cumulative TASK-040/TASK-042 amendment. No publication becomes approved by its authoring commit.
...
All three earlier gaps are closed. TASK-040 closes the architectural ownership gap for both merge effects. TASK-042 returns the tasks-owned predicate correction and the unresolved policy-attestor control-plane dependency; implementations and activation controls remain deliberately absent behind TASK-044 and the HUMAN-004 prerequisites.
...
For the two merge executors, ordinary branch validation is necessary but insufficient. Their immutable activation records additionally require independent implementation review, security validation, negative-capability and failure-injection evidence, live GitHub protection tests, the immutable tasks-owned gate-vocabulary correction, and a complete current signed policy attestation from the separate human-controlled control plane. A missing, incomplete, stale, revoked, or drifting member refuses. TASK-040/TASK-042 supply the cumulative contract only; TASK-044 reviews it, and the Orchestrator alone may consider later work after a passing verdict.

### Exact published-head verification obligation

Every artifact owner designates one full final authored commit and reruns every declared target-dependent check with `HEAD` equal to that commit. Target-dependent includes assignment/settings reads; scope and diff classification; framework, orchestration, security, and architecture fixtures; whitespace; links and ADRs; task/relation/topology/HUMAN-004 enumerations; remote/PR identity; and GitHub check state. Each durable result names the full commit, exact command and material arguments, resolved base, and actual result. Derived counts are enumerated from that commit's tree under MC-011.

A later content commit invalidates the complete earlier target-dependent set, even when the commit only edits owner evidence. The owner must repeat the set and replace the handoff evidence before publication. Pull-request-body and external handoff updates made after the final authored commit do not change the target tree and may record those results. After push, remote branch and PR head must equal the verified commit. GitHub checks are recorded as actually present and successful, present and non-passing, or absent; an empty rollup is absent and never inferred successful.

This is owner evidence, not gate approval. It applies to every role and artifact and is returned as exact human-controlled `AGENTS.md` text in [POST-GATE-MERGE-EXECUTORS.md](POST-GATE-MERGE-EXECUTORS.md#exact-returned-agentsmd-amendment). TASK-042 applies it to its own final authored head before handoff.
