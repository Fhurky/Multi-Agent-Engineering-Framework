...
Normative scheduling contract for the autonomous runtime. Produced under TASK-002 and amended under TASK-016, TASK-024, and TASK-028. Related decisions: [ADR-0005](../../adr/0005-time-bounded-leases-with-monotonic-fencing-tokens.md) and [ADR-0015](../../adr/0015-typed-scheduling-gate-and-activation-contracts.md), as amended by [ADR-0017](../../adr/0017-durable-ingress-inbox-and-ingress-epochs.md), [ADR-0018](../../adr/0018-publication-classes-and-gate-lineages.md), and TASK-028 ADRs [0024](../../adr/0024-task-record-projection-contract.md), [0029](../../adr/0029-ingress-delivery-ownership.md), and [0031](../../adr/0031-pre-dispatch-ingress-observer-and-collector.md). Implemented by TASK-005, over an inbox and collector implemented by TASK-026.
...
`G*` is acyclic if and only if the graph is schedulable. The **eight** no-deadlock invariants below apply to the current projected committed graph (revision 7 at TASK-028). Invariants 1 through 5 originated under TASK-016 except where later edge forms changed; 6 through 8 were added under TASK-024 and their pair-property projection was completed under TASK-028.
...
The proof is restated for the committed graph current at TASK-028 authoring time, loaded through `TaskRecordProjection`, so the claim, fixture, and runtime graph are the same graph.

Take the topological order

```text
HUMAN-001, TASK-001, TASK-014, TASK-021, TASK-022, TASK-023, TASK-027, TASK-030,
TASK-002, TASK-015, TASK-016, TASK-020, TASK-024, TASK-025, TASK-028, TASK-029,
TASK-018, TASK-019, TASK-003, TASK-004, TASK-017, TASK-026, TASK-005, TASK-006,
TASK-007, TASK-008, TASK-009, TASK-010, TASK-011, TASK-012, TASK-013
```

- **Invariants 1 and 5.** Every task appears after all of its dependencies and after the pre-merge gate owner of every task it integrates. The architecture lineage edge has a floor at lineage round 4 and expands to TASK-029 before every implementation holder. TASK-012's runtime-QA lineage edge expands to TASK-011 before TASK-012. No back edge exists, so `G*` is acyclic.
- **Invariant 2.** The tasks holding `gateFor` entries are TASK-009 … TASK-012, TASK-014, TASK-015, TASK-019 … TASK-023, TASK-025, TASK-027, TASK-029, and TASK-030. Every edge each holds to a task it gates is `review_ready`. TASK-012's only non-`review_ready` edge names `LIN-RUNTIME-QA`, of which TASK-012 records no round. TASK-029 and TASK-030 each record a lineage round and hold no `gate_passed` edge naming that lineage.
- **Invariant 3.** Forty-six pairs, each agreeing across both sides on all eight fields after projection.
- **Invariant 4.** The only non-empty `preMergeGates` are TASK-002 `review`, TASK-016 `review`, TASK-024 `review`, TASK-028 `review`, and TASK-018 `review`. None of TASK-015, TASK-019, TASK-020, TASK-025, or TASK-029 holds an `integrated` edge to any target it gates.
...
- **Invariant 7.** Every pair carries the scheduling properties and projected verdict on both sides. `retrospective` is `false` for eleven pairs and `true` for 35; `gateClass` is `point` for eighteen pairs and `aggregate` for 28, each recomputed against the publication of the artifact that round reviews.
- **Invariant 8.** Eight lineages are registered. `LIN-DECOMP-REVIEW` declares rounds 1 through 7; `LIN-ARCH-REVIEW` declares rounds 1 through 4 over the growing cohort TASK-002 → TASK-016 → TASK-024 → TASK-028; the remaining six declare round 1 only. Every target named by a pair is a cohort member, and every round greater than 1 follows a recorded verdict.

This proof is a property of the projected committed graph, not of this document. TASK-005's `validateGraph` recomputes all eight invariants at load and on every admission; fixture assertions derive counts from the records rather than freezing this prose as a second graph.
...
10. **Projection agreement.** TASK-007 loads every committed record as the exact `TaskRecordDocument` schema, TASK-005 projects each through `TaskRecordProjection`, and the fixture asserts successful projection, canonical round-trip of every projected source field, and `validateGraph.ok` over the whole projected set. No test claims that snake_case document fields equal camelCase runtime fields name-for-name.
...
15. **Projected committed graph loads.** Load the committed task records through the TASK-007 parser and TASK-005 projection as a fixture and assert `validateGraph` returns `ok`, with the eight invariants each exercised by at least one pair of the real graph.
