...
Added under TASK-016, amended under TASK-024, and corrected under TASK-028. Committed records do not compile into runtime types name-for-name. TASK-007 parses the exact snake_case `TaskRecordDocument` schema; TASK-005 owns the only legal, pure `TaskRecordProjection` into `TaskProposal`; and `TaskCreated` is the only operation that creates a `TaskRecord`. The source-to-target mapping and optional-field defaults are normative in [INTERFACE-CONTRACTS.md](INTERFACE-CONTRACTS.md#2c-committed-task-record-projection--declared-by-task-003-implemented-by-task-005-and-task-007).
...
The decision is total over the one canonical `ReconciliationInput` declared in [INTERFACE-CONTRACTS.md](INTERFACE-CONTRACTS.md). ADR-0030 owns its cardinality. The type includes task state, lease state, reduced ledger state, deadline state, activation presence, and attempt-matching adoptable-result presence; no row may read an undeclared axis. When a task's current attempt has several ledger entries, input construction reduces them before `decide`: intended non-idempotent wins, otherwise intended idempotent, otherwise committed, otherwise none. `mixed` is a construction fixture, not a domain member.
...
| R1 | `running` | absent | `committed` / present | any | `adopt` | `WorkerSucceeded{ result, proposedTasks }` built from `AdoptableResult` | `succeeded` |
| R1x | `running` | absent | `committed` / absent | any | `escalate` | `TaskBlocked{ reason: 'unreconstructable_result:<effectId>' }` | `blocked` |
...
1. **Total decision table.** Enumerate every point of the canonical `ReconciliationInput` domain whose cardinality ADR-0030 declares. Assert exactly one decision is produced and that its emitted sequence is legal from that pre-batch state. No point may produce two decisions, zero decisions, or an `IllegalTransition`. Separately construct a current attempt with mixed ledger entries and assert input reduction yields the mandated four-value `ledgerState` member before `decide` is called.
...
11. **One verdict, every relation.** Use TASK-029's four projected relations as the fixture and record one `GateVerdictRecorded`. Assert four `GateVerdictRecord` entries appear, one per target; that all four relations close or all four stay open; that the four relations at lineage round 4 receive the same verdict; and that no event exists that could record a verdict for a subset.
