...
Normative component decomposition for the autonomous multi-agent runtime. Produced under TASK-002 and amended under TASK-016, TASK-024, and TASK-028. Related decisions: [ADR-0002](../../adr/0002-runtime-component-boundaries-and-module-ownership.md) as superseded in part by [ADR-0011](../../adr/0011-agent-workspace-lifecycle-module.md) and [ADR-0021](../../adr/0021-durable-ingress-module-and-the-eight-module-map.md), plus [ADR-0014](../../adr/0014-live-run-control-and-process-tree-ownership.md) and TASK-028 ADRs [0024](../../adr/0024-task-record-projection-contract.md), [0028](../../adr/0028-nominal-store-issued-durable-append-receipts.md), [0029](../../adr/0029-ingress-delivery-ownership.md), and [0031](../../adr/0031-pre-dispatch-ingress-observer-and-collector.md).
...
|---|---|---|---|---|
| Inbox durability, external-fact validation, deduplication, authorized append, and the HUMAN-002 collector | **TASK-026**, ingress | `IngressInbox`, `PreDispatchIngressValidator`, `PreDispatchIngressCollector` | TASK-005 receives only the resulting high-water signal; no other module appends | Store invariants and the collector's append transaction share one owner, so no bypass path can skip identity-keyed deduplication or principal checks |
...
|---|---|---|
| `src/orchestrator/state/contracts/` | TASK-003 | Identifiers, exact task-record document and projection interfaces, runtime records and states, journal and event unions, store interface, typed graph contracts, ingress contracts, **nominal durable append receipts and verifier**, control channel, workspace lifecycle |
...
|---|---|---|---|
| `RunId`, `TaskId`, `Sha256Hex`, `IsoTimestamp`, `FencingToken`, `TaskProposal`, and — under TASK-016 — `InvocationId`, `WorkspaceId`, `InvocationRecord`, `ProcessTreeOutcome`, and — under TASK-028 — `AgentProcessRegistrationSubject` plus `AgentReceiptVerifier` | `agents/contracts` | A narrowed structural view of the state verifier | Lets TASK-004 compile in parallel with TASK-003 with no import edge; the supervisor composition adapter narrows the state verifier, while the nominal brand is neither copied nor made falsely interchangeable |
...
There are still exactly **two** permitted duplications. TASK-028 adds the narrowed process-registration subject and verifier view to the first alias list while removing every duplicated receipt type; it does not open a third duplication. A duplication is a pair of roots holding the same structural shape, and the nominal receipt deliberately has no second declaration.

The primitive and verifier views are structurally identical, but the nominal receipt itself is declared only once. Independently declaring the same `unique symbol` in both roots would create different nominal types and make them non-interchangeable; TASK-028 explicitly prohibits that duplication. The verifier accepts `unknown` and returns a structural subject, so a state-issued receipt crosses into TASK-004 without a cast while a synthesized object still fails store verification. Neither permitted duplication is compiler-checked, so both remain review obligations. A third duplication requires a new ADR.
