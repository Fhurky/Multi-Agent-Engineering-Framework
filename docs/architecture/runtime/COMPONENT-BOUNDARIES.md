...
Normative component decomposition for the autonomous multi-agent runtime and its release control plane. Produced under TASK-002 and amended under TASK-016, TASK-024, TASK-028, TASK-032, TASK-036, TASK-040, TASK-042, and TASK-046. Related decisions: [ADR-0002](../../adr/0002-runtime-component-boundaries-and-module-ownership.md) as superseded in part by [ADR-0011](../../adr/0011-agent-workspace-lifecycle-module.md) and [ADR-0021](../../adr/0021-durable-ingress-module-and-the-eight-module-map.md), plus [ADR-0014](../../adr/0014-live-run-control-and-process-tree-ownership.md), TASK-028 ADRs [0024](../../adr/0024-task-record-projection-contract.md), [0028](../../adr/0028-nominal-store-issued-durable-append-receipts.md), [0029](../../adr/0029-ingress-delivery-ownership.md), and [0031](../../adr/0031-pre-dispatch-ingress-observer-and-collector.md), TASK-032 ADRs [0032](../../adr/0032-lossless-task-record-source-and-projection.md), [0033](../../adr/0033-unique-committed-result-effect-recovery.md), and [0034](../../adr/0034-spawn-owned-registration-proof-refusal.md), TASK-036 ADRs [0038](../../adr/0038-total-pre-dispatch-ingress-append-dispositions.md) and [0039](../../adr/0039-single-nominal-receipt-authority-and-cross-root-conformance.md), TASK-040 [ADR-0042](../../adr/0042-conditionally-authorized-post-gate-merge-executors.md), TASK-042 [ADR-0043](../../adr/0043-exact-merge-admission-policy-attestation-and-published-head-evidence.md), and TASK-046 [ADR-0044](../../adr/0044-single-policy-result-and-two-phase-published-head-evidence.md).

## Correction register — TASK-046

TASK-046 adds no module, source path, contract root, or import edge. It removes the contradictory executor-side policy-read description from the component diagram and reaffirms this boundary: executor identities can read immutable PR, check, head, and base state and can invoke only their one exact merge endpoint. Separately controlled observer principals alone perform Administration/ruleset/bypass policy reads through `RepositoryPolicyAttestor`; executors receive only signed exact-subject fresh attestations. TASK-044 recorded `changes-required`, and implementation task creation remains blocked behind a passing TASK-047 or later verdict.
...
3. There are no cycles. The graph is a strict partial order. Future implementation-task waves are not invented by this amendment; only the Orchestrator may consider them after a passing TASK-047 or later verdict.
...
Its source is `src/orchestrator/integration/`; its tests are `tests/unit/orchestrator/integration/`. Both fit the existing runtime write scope. Its implementation task does not exist and remains blocked behind a passing TASK-047 or later verdict. Its repository reads are limited to immutable PR, check, head, and base state; it owns no policy-observation port. Its complete contract is [POST-GATE-MERGE-EXECUTORS.md](POST-GATE-MERGE-EXECUTORS.md).
...
Its source and nested tests are under `scripts/release/integration-merge/`, within the existing DevOps write scope. Its implementation task does not exist and remains blocked behind a passing TASK-047 or later verdict. Its repository reads are limited to the immutable release manifest, PR, check, head, and base state; it owns no policy-observation port. Its complete contract is [POST-GATE-MERGE-EXECUTORS.md](POST-GATE-MERGE-EXECUTORS.md).
...
GitHub's documented ruleset response omits `bypass_actors` without ruleset write access, so an acceptable executor credential cannot implement this boundary. The boundary remains unprovisioned. After non-policy activation prerequisites are valid, verified missing provisioning selects HUMAN-004's third exception; an unknown cause selects the unclassifiable candidate-third exception; only an operational outage with credential or repository-policy action excluded returns `PolicyObservationUnavailable`. Every result keeps activation closed. Because the boundary is external and human-controlled, it adds zero module rows, zero source paths, zero graph nodes, zero import edges, and zero contract roots.
...
|---|---|
| Every existing component maps to one implementation task, and each proposed executor maps to one future owner role without shared ownership | Module map and contract roots above; task creation remains deferred behind a passing TASK-047 or later verdict |
