...
Normative cross-module contracts for the autonomous runtime. Produced under TASK-002 and amended under TASK-016, TASK-024, and TASK-028. This document exists so that TASK-003 through TASK-008, TASK-017, and TASK-026 never negotiate an interface during execution.
...
export type PreDispatchCollectResult =
  | { ok: true; entry: IngressEntry; highWaterMark: IngressSeq }
...
## 2c. Committed task-record projection — declared by TASK-003, implemented by TASK-005 and TASK-007

TASK-028 withdraws the claim that committed YAML loads into `TaskRecord` without field renaming. TASK-007 owns file I/O and YAML parsing through `TaskRecordDocumentSource`; TASK-005 owns the pure, total `TaskRecordProjection`. No other module may parse a task record or construct a proposal from one.
...
export interface GateRelationDocument {
  task: TaskId;
  gate: GateName;
  round: number;
...
export interface ActivationDocument {
  mode: 'event-triggered';
  subscribed_event_types: IngressEventType[];
  last_consumed_event_seq: IngressSeq;
  state: ActivationSpec['state'];
  bootstrap_dispatch_contract: BootstrapDispatchContract;
}

/** Exact projected/checked vocabulary of YAML front matter. TaskRecordDocumentSource places
 *  every other top-level key into retained without renaming or interpretation. */
export interface TaskRecordDocument {
  task_id: TaskId;
  title: string;
  ...
  gate_tasks?: GateRelationDocument[];
  gate_for?: GateRelationDocument[];
...
export type TaskRecordDocumentLoadResult =
  ...
  | { ok: false; error: 'YamlInvalid' | 'RequiredFieldMissing' | 'UnknownProjectedShape'; path: string; field: string };

export interface TaskRecordProjection {
  /** Pure and total over TaskRecordDocument. It returns a proposal, never a TaskRecord. */
  project(record: TaskRecordDocumentEnvelope): TaskRecordProjectionResult;
}

export type TaskRecordProjectionResult =
  | { ok: true; proposal: TaskProposal; retained: Readonly<Record<string, unknown>> }
  | { ok: false; error: 'DerivedFieldMismatch' | 'ProjectionInvalid' | 'GRAPH_GATE_VERDICT_DECLARATION_MISMATCH'; path: string; field: string; detail: string };
```

The mapping is exact:

| Document source | Runtime target |
|---|---|
| `task_id`, `title`, `owner_role`, `llm`, `write_scope` | `taskId`, `title`, `ownerRole`, `llm`, `writeScope` |
| `dependencies` | `TaskDependency[]`, with only `lineage_round -> lineageRound` renamed |
| `required_gates`, `pre_merge_gates` | `requiredGates`, `preMergeGates` |
| `gate_tasks`, `gate_for` | `gateTasks`, `gateFor`; omission projects to `[]`; `gate_class`, `gate_lineage`, `lineage_round`, and `verdict` map to declared relation fields |
| `resource_lock`, `parent_task`, `publication_class`, `max_attempts` | Optional lock/parent omissions project to `null`; `publication_class` maps to `publicationClass`; `max_attempts` maps to `maxAttempts` when present, while omission remains `undefined` so `TaskCreated` applies `RuntimeConfig.maxAttempts` |
| `activation` | Omission projects to `null`; when present it maps to `ActivationSpec`, including required `bootstrap_dispatch_contract -> bootstrapDispatchContract` |
| envelope `path` | `recordPath` |
| `branch`, `worktree` | Derived from `(llm, ownerRole, taskId)` and compared; mismatch rejects, neither value is consumed |
| `status` and every remaining key | Retained verbatim and inert for scheduling |

The fixture obligation loads every committed record, projects it, round-trips every projected field under canonical JSON, checks every derived field, and validates the complete projected graph. `TaskCreated` is the only operation that adds runtime-only state such as attempt, lease, timestamps, publication, integration, and verdict registers.
...
`RuntimeEvent` remains a closed union of **52** members under TASK-028. `EffectIntentRecorded` changes shape but is not a new member. Adding a member is a contract change and requires an ADR amendment.
...
**TASK-028 event guards.** Applying `EffectIntentRecorded` copies `isTaskResultEffect` into the new ledger entry. For one `(taskId, attempt)`, a second event with the flag `true` is `IllegalTransition/GuardFailed`; the first is legal only after a matching `run.pendingResults[taskId]` is durable. The uninterrupted order is `WorkerResultRecorded -> EffectIntentRecorded -> effect execution -> EffectCommitted -> WorkerSucceeded`, and recovery's `adopt` path emits only the last event because the first four facts are already durable.
...
export interface Scheduler {
   ...
   * no-deadlock invariants of the current projected committed graph (revision 7 at TASK-028), including
...
When several ledger entries exist for the current attempt, input construction reduces them
before calling `decide`: any intended non-idempotent entry wins, otherwise any intended entry
yields `intended_idempotent`, otherwise a committed entry yields `committed`. The constructed
`mixed` fixture tests this reduction; `mixed` is neither another input member nor another axis.
