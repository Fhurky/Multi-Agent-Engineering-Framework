...
Normative durability contract for the autonomous runtime. Produced under TASK-002 and amended under TASK-016, TASK-024, and TASK-028. Related decisions: [ADR-0004](../../adr/0004-durable-state-as-event-journal-with-atomic-checkpoints.md) as superseded in part by [ADR-0012](../../adr/0012-crash-atomic-journal-batches-with-commit-records.md), plus [ADR-0017](../../adr/0017-durable-ingress-inbox-and-ingress-epochs.md), [ADR-0019](../../adr/0019-durable-intent-receipts-for-side-effects.md), and TASK-028 ADRs [0025](../../adr/0025-result-effect-identity-in-the-event-union.md) and [0028](../../adr/0028-nominal-store-issued-durable-append-receipts.md). Implemented by TASK-003, with the ingress store implemented by TASK-026.

## Amendment register — TASK-028

| Superseded claim (TASK-024) | Superseded by | Finding | Decision |
|---|---|---|---|
| A base structural receipt carrying only event and journal identity proved durable issuance | A discriminated receipt union with event-specific subjects, a state-root-only nominal brand, and a store-verifiable current-epoch proof | A-206 | [ADR-0028](../../adr/0028-nominal-store-issued-durable-append-receipts.md) |
| Result-effect identity was not carried by the event that creates the ledger row | `EffectIntentRecorded.isTaskResultEffect` is persisted and uniqueness-checked during append | A-203 | [ADR-0025](../../adr/0025-result-effect-identity-in-the-event-union.md) |
...
Step 8's `receipts` is amended under TASK-028. The array contains one discriminated `DurableAppendReceipt` per event, in batch order. Specialized members carry the exact store-recorded subject required by their boundary: `invocationId`, `workspaceId` plus intent, or `workspaceId` plus `publishedCommit`. The state root alone declares the nominal brand and issues an opaque proof after the commit-record fsync. `verifyReceipt(receipt: unknown)` returns the retained subject only when that proof was issued by this store in the current writer epoch; otherwise it returns `null`. A receipt remains evidence, never write authority.

For `EffectIntentRecorded`, append copies `isTaskResultEffect` into the ledger entry and rejects a second true value for the same `(taskId, attempt)` or a true value without a matching durable `WorkerResultRecorded`. That semantic validation runs before journal bytes are written, so a committed batch can never introduce two candidate result effects for one attempt.
