...
Normative provider contract for the autonomous runtime. Produced under TASK-002 and amended under TASK-016, TASK-024, TASK-028, TASK-032, and TASK-036. Related decisions: [ADR-0007](../../adr/0007-provider-adapter-boundary-and-error-taxonomy.md), [ADR-0014](../../adr/0014-live-run-control-and-process-tree-ownership.md), TASK-028 ADRs [0026](../../adr/0026-blocked-drain-attach-and-unverified-closure-recovery.md), [0028](../../adr/0028-nominal-store-issued-durable-append-receipts.md), and [0029](../../adr/0029-ingress-delivery-ownership.md), [ADR-0034](../../adr/0034-spawn-owned-registration-proof-refusal.md), and [ADR-0040](../../adr/0040-typed-provider-planning-result-and-phase-observables.md). Implemented by TASK-004.

## Amendment register — TASK-036

| Superseded contract or observable (TASK-034) | Replaced by | Finding | Decision |
|---|---|---|---|
| `planInvocation` documented classified `UNKNOWN_PROVIDER_FAMILY` failure but returned only `WorkerInvocationPlan` | `WorkerPlanResult` returns either the plan or the exact classified failure before registration | A-505 | [ADR-0040](../../adr/0040-typed-provider-planning-result-and-phase-observables.md) |
| Observable rows assigned timeout and workspace refusal to withdrawn `execute` | Plan, begin, and complete each have phase-accurate observables; preparation failure is tested at the workspace/dispatch producer boundary | A-505 | [ADR-0040](../../adr/0040-typed-provider-planning-result-and-phase-observables.md) |

## Amendment register — TASK-032

| Superseded claim | Superseded by | Finding | Decision |
|---|---|---|---|
| `spawnOwned` promised `RegistrationNotDurable` while `SpawnOwnedResult` could express only success or `AdapterFailure` | Both `beginInvocation` and the actual `spawnOwned` side-effect boundary return the shared `RegistrationNotDurableRefusal` before any group or process exists | A-206 (A-102 view) | [ADR-0034](../../adr/0034-spawn-owned-registration-proof-refusal.md) |
...
```text
supervisor  ->  AgentWorker.planInvocation(assignment)          provider-neutral WorkerPlanResult
                    |-- failed: classified; append/spawn nothing
                    '-- ok: WorkerInvocationPlan
...
|---|---|---|---|
| 1 | supervisor | `planned = worker.planInvocation(assignment)`. Pure: resolves the family, computes `invocationId` deterministically from `(runId, taskId, attempt, idempotencyKey)`, derives the group name, and assembles the command vector. Spawns nothing. `!planned.ok` records the classified failure and stops | — |
| 2 | supervisor | On `planned.ok`, set `plan = planned.plan` and append the registration | `ProcessGroupRegistered { invocationId, taskId, attempt, groupKind, groupName }`, durable **before** the spawn |
| 3 | supervisor | Take `receipt` from the append result | — |
| 4 | supervisor | `begun = await worker.beginInvocation(plan, receipt, signal)`. The worker verifies the opaque value through `AgentReceiptVerifier`; its internal call to `spawnOwned` returns the same typed `RegistrationNotDurableRefusal` for a missing, forged, stale-epoch, or subject-mismatched receipt | — |
...
6. **Spawn proof refusal is representable at both boundaries.** Call both `beginInvocation` and `spawnOwned` with a missing value, a forged proof, a current proof naming another `invocationId`, and a proof from a superseded `writerEpoch`. Assert every call returns the `RegistrationNotDurableRefusal` union member—not `AdapterFailure`—and that no group or process was created, asserted on the OS process table.
...
### Test obligations added for A-505 under TASK-036

10. **Typed unknown-family planning failure.** Call `planInvocation` with an unregistered lower-case family. Assert the exact `WorkerPlanResult` failure member: `invalid_request`, `UNKNOWN_PROVIDER_FAMILY`, null provider code, null retry-after, and a redacted English message. Assert it does not throw, no `ProcessGroupRegistered` is appended, and no group or process exists.
11. **Plan success is the only registration input.** For every registered fake family, assert `planInvocation` returns `{ ok: true, plan }`; only `planned.plan` may supply the registration subject and the input to `beginInvocation`.
12. **Phase ownership.** With a provider that never resolves, assert `completeInvocation` — not a withdrawn `execute` and not `planInvocation` — returns a timeout-classified worker result within the bound and closes the tree. The workspace suite separately proves that a failed preparation constructs no assignment and calls none of the three worker phases.
...
1. **`planInvocation`.** Resolve the adapter by `invocation.llm`. Return `{ ok: false, failure: { failureClass: 'invalid_request', code: 'UNKNOWN_PROVIDER_FAMILY', providerCode: null, retryAfterMs: null, ... } }` for an unregistered family; throwing is non-conforming. On success return `{ ok: true, plan }`, with deterministic `invocationId`, group name, invocation, and command vector. The worker reads the named role/task files but interprets no governance. The phase spawns no process, creates no group, and appends nothing.
...
The public worker surface has exactly these three phases. No observable is assigned to the withdrawn `execute` operation.

| Public phase | Observable post-condition |
|---|---|
| `planInvocation` | Returns `WorkerPlanResult` without throwing. Unknown family is the exact classified `UNKNOWN_PROVIDER_FAMILY` failure and causes no registration append, group creation, process spawn, timer, or provider call. Success carries one deterministic `WorkerInvocationPlan` |
| `beginInvocation` | Without matching verified registration evidence, returns `RegistrationNotDurable` and creates no group or process. With evidence, either returns the binding/handle or a classified spawn failure; every spawned process is in the owned group |
| `completeInvocation` | Awaits the provider under the task timeout, classifies timeout and provider outcomes, performs bounded verified tree closure, and returns an addressable `WorkerResult`. It appends no close event itself |
...
The failed-workspace assertion is deliberately absent here. The worker cannot receive such an assignment because `WorkspaceRef.prepared` is the literal `true`; [WORKSPACE-LIFECYCLE.md](WORKSPACE-LIFECYCLE.md#test-obligations) owns the upstream fixture that refuses assignment construction after preparation failure.
