...
- Status: Accepted; superseded in part by [ADR-0027](0027-finalize-split-around-the-publication-append.md), which splits finalize around the durable publication append; [ADR-0028](0028-nominal-store-issued-durable-append-receipts.md), which replaces structural base receipts with discriminated nominal and store-verifiable proofs; and [ADR-0040](0040-typed-provider-planning-result-and-phase-observables.md), which replaces the plan phase's direct plan return with `WorkerPlanResult`. Every other decision below stands.
- Date: 2026-08-05
