...
- Status: Accepted; superseded in part by [ADR-0020](0020-durable-adoptable-results-for-recovery.md), which replaces the `adopt` row's reliance on `resultDigest`, and by [ADR-0030](0030-one-canonical-recovery-decision-input-domain.md), which replaces the four-input cardinality with the canonical `ReconciliationInput`. The one-decision-per-task invariant, priority order, `current_epoch` defect rule, determinism rule, and `attemptStartedAt` stand.
- Date: 2026-08-05
