...
- Status: Accepted; superseded in part by [ADR-0020](0020-durable-adoptable-results-for-recovery.md), which replaces the `adopt` row's reliance on `resultDigest`; by [ADR-0030](0030-one-canonical-recovery-decision-input-domain.md), which replaces the four-input cardinality with the canonical `ReconciliationInput`; and by [ADR-0037](0037-recovery-completion-evidence-and-decision-block-legality.md), which replaces one-event-per-task and restored-state-only legality wording with sequential decision-block legality. The one-decision-per-task invariant, priority order, `current_epoch` defect rule, deterministic block order, and `attemptStartedAt` stand.
- Date: 2026-08-05
